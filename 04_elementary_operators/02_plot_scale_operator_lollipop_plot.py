...
#############################################################################################################################
## A sequence x[n], is scaled by a factor a belonging to a family of complex numbers 
## y[n] = a*x[n]
## If a is real, then the scaling represents a simple amplification or attenuation of the signal (when a > 1 and a < 1, respectively).  
## If "a" is complex, amplification and attenutation are compounded with a phase shift.
#############################################################################################################################
def f(x):
   return math.sin(x) 
...
def plotGraph():
  ...
  ax = plt.axes()
  #ax.xaxis.grid(True)
  plt.legend((line1,line2),("f(x) = sin(x)","g(x) = 3sin(x)"),loc="upper left")
  plt.show()
